...
class TTSClient:
    def __init__(self):
        ...
        if os.path.exists(key_path) and os.path.getsize(key_path) > 0:
            try:
                ...
                print(f"TTS: Error loading {key_path} - {e}. Using Default Credentials.")
                self.credentials = None
        else:
            print(f"TTS: {key_path} not found or empty. Using Default Credentials (Cloud Run)...")
            self.credentials = None 
    ...
    def generate_audio(self, text, voice_name="en-US-Journey-D"):
        ...
        try:
            ...
            # 5. Execute Request
            print(f"TTS Request: Voice={voice_name}, Lang={computed_lang}, TextLen={len(text)}")
