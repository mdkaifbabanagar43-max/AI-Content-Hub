...
from core.services.canonical_generation_engine import CanonicalGenerationEngine, VideoGenerationRequest
...
async def run_direct_test():
    ...
    print(f"QUALITY_REVIEW_ESCALATE: {ModelRoutingConfig.QUALITY_REVIEW_ESCALATE}")
    print(f"SOURCE_ANALYSIS: {ModelRoutingConfig.SOURCE_ANALYSIS}")
    print(f"NORMAL_STORY: {ModelRoutingConfig.NORMAL_STORY}")
    print(f"COMPLEX_STORY: {ModelRoutingConfig.COMPLEX_STORY}")
    ...
    blueprint = ProductionBlueprint(**bp_data)
    engine = CanonicalGenerationEngine()
    req = VideoGenerationRequest(blueprint=blueprint)
    
    print("🚀 Starting direct CanonicalGenerationEngine test...")
    try:
        response = await engine.generate_video(req, user_id)
        print("✅ Generation Complete!")
        print("Response:", json.dumps(response.model_dump(), indent=2))
        
        if response.final_video_url:
            print(f"Final Video URL: {response.final_video_url}")
