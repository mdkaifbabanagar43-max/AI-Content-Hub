...
export default function VoiceLab({ onNavigate }: { onNavigate?: (tab: string) => void }) {
            ...
            <div className="w-1/2 flex flex-col gap-6">
                ...
                <div className="flex-1 bg-zinc-900/50 border border-zinc-800 rounded-3xl p-6 relative group focus-within:ring-2 focus-within:ring-cyan-500/50 transition-all">
                    <textarea
                        ...
                        className="w-full h-full bg-transparent resize-none outline-none text-lg text-white font-mono placeholder:text-zinc-500"
                        maxLength={5000}
                    />
                    <div className="absolute bottom-4 right-6 text-xs font-mono text-zinc-400 group-focus-within:text-cyan-500">
                        {script.length} / 5000
                    </div>
                </div>
                ...
                <button
                    ...
                    className="w-full py-5 bg-gradient-to-r from-purple-600 via-purple-500 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold text-lg rounded-2xl shadow-lg shadow-purple-500/20 transition-all transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
                >
            ...
            <div className="w-1/2 flex flex-col">
                <div className="flex-1 bg-zinc-950 border border-zinc-800 rounded-3xl relative overflow-hidden flex flex-col items-center justify-center p-12">
                    ...
                    <div className="flex items-center gap-1 h-32 mb-8">
                        {Array.from({ length: 40 }).map((_, i) => (
                            <motion.div
                                ...
                                initial={{ height: 4 }}
                                animate={{
                                    height: isGenerating || isPlaying ? [8, 64, 16, 48, 8] : 4,
                                    backgroundColor: isGenerating || isPlaying ? '#a855f7' : '#3f3f46'
                                }}
                                transition={{
                                    repeat: Infinity,
                                    duration: 1,
                    ...
                    <div className="text-center z-10">
                        {isGenerating ? (
                            <p className="text-purple-400 font-mono animate-pulse">Processing Neural Audio...</p>
                        ) : audioUrl ? (
                            <div className="flex flex-col items-center animate-in slide-in-from-bottom-5">
                                <div className="w-20 h-20 bg-purple-600 rounded-full flex items-center justify-center text-white mb-4 shadow-lg shadow-purple-500/50">
                                    <Disc size={40} className={isPlaying ? "animate-spin" : ""} />
                                </div>
                                <h3 className="text-2xl font-bold text-white mb-2">Audio Ready</h3>
                                <p className="text-zinc-400 mb-6">Generated with {voiceModel === 'Clone' ? 'Your AI Voice' : voiceModel}</p>
                            </div>
                        ) : (
                            <p className="text-zinc-500 font-medium">Ready to Synthesize</p>
                        )}
                    </div>
                </div>
                ...
                {audioUrl && (
                    ...
                    >
                        <button
                            onClick={togglePlay}
                            className="w-12 h-12 bg-purple-600 rounded-full flex items-center justify-center text-white hover:bg-purple-500 transition-colors"
                        >
                            {isPlaying ? (
                        ...
                        <div className="flex-1">
                            <div className="h-10 bg-zinc-950 rounded-lg overflow-hidden relative">
                                ...
                                <div className="absolute top-0 left-0 bottom-0 w-1/3 bg-purple-500/20 border-r-2 border-purple-500" />
                                <div className="absolute inset-0 flex items-center justify-between px-3">
                                    <span className="text-xs font-mono text-purple-400">0:12</span>
            ...
            {showPremiumModal && (
                ...
                >
                    ...
                    >
                        ...
                        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center shadow-lg shadow-purple-500/25 mx-auto">
